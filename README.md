...
- [功能](#功能)
- [使用教程](使用教程.md)
- [下载 Word 版使用教程](Cell_CNS_Figure 使用教程.docx)
- [安装](#安装)
- [使用](#使用)
- [凭据和费用](#凭据和费用)
- [验证与当前边界](#验证与当前边界)
...
产品流程：第一阶段布局设计 5 次额度；第二阶段高级设计另计 20 次；两阶段合计 25 次。收到合格研究文字后直接进入第一阶段费用确认。每阶段确认时显示实时余额及“开始设计后不退额度”，拒绝第二阶段则交付图 A 后结束。

计费规则：第一阶段仅提交研究文字，扣除 5 次额度；第二阶段提交研究文字与图 A，扣除 20 次额度。两阶段合计 25 次。提交成功即扣费，开始设计后不退额度，领取结果不重复扣费。
